...
%         legend('情形1','情形2');
       ...
       %% --------------------频率不变波束形成设计--------------------%%
        %-----------------频率信息-------------------------%        
%         f_index=[linspace(fl,6000,4),linspace(6000,fh,12)];     %子带划分
%         f_index=linspace(fl,fh,12);

        f_index=linspace(fl,fh,16);
...
%         beam_FIB_SOCP=beam_FIB_SOCP.*beam_FIB_SOCP;
        ...
        %-----------------不同频率分量对应的主瓣均方根误差------------------------------%
        plot(f_index/fh*pi,10*log10(errors)-10*log10(length(ang_mainlobe))*ones(size(errors)),'k^:');%+,*,。,x,s（方块),d（菱形）,p（五角形）
