...
%         close all
       ...
       %% **************************Dolph-Chebyshev加权设计期望波束响应 **************************%
        ...
        array_num=11;                          %设计期望波束响应采用的参考阵元数
        
        vec=-5:5;                        %期望波束响应时的导向矢量是以中间阵元为参考点来计算的，因为共有11个，故vec取为-5:5
...
%         legend('情形1','情形2');
        ...
        %% *************** 频率不变波束形成 *************** %%
        Omega_range=2*pi*linspace(fl,fh,15);                    %子带划分,得到w的范围为15个窄带子带
        
%         Omega_range=2*pi*linspace(fl,fh,12);                    %子带划分,w的范围
%         Omega_range=2*pi*[linspace(fl,6000,4),linspace(6000,fh,12)];     %子带划分
        ...
        %-----------------不同频率分量对应的主瓣均方根误差------------------------------%      
        plot(Omega_range/2/fh,10*log10(errors)-10*log10(length(mainlobe))*ones(size(errors)),'k--.');%+,*,。,x,s（方块),d（菱形）,p（五角形）
        
        h = gca;
        set(h,'FontSize',10,'FontName','宋体');
        set(h,'FontName','Times New Roman');
        
%         title('不同频率分量对应的主瓣均方根误差');
        ylabel('主瓣均方根误差(dB)');
        xlabel('数字频率');
