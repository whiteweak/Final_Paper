...
function Patt3d(beam_abs,p_scl)
% function Patt3d，参数p_scl控制波束图的表现方式
...
theta=((0:del_th:90))*pi/180;

phi=(-180:del_phi:180)*pi/180;
...
beam_temp=beam_abs(:,1);
[XCT1,YCT1,ZCT1]=CrossSecT(thetac1,phi,ratio,scl,rmin,beam_temp);

thetac2=ones(size(theta))*theta(length(theta));
% [XCT2,YCT2,ZCT2]=CrossSecT(thetac2,phi,ratio,scl,rmin);
beam_temp=beam_abs(:,length(theta));
[XCT2,YCT2,ZCT2]=CrossSecT(thetac2,phi,ratio,scl,rmin,beam_temp);
...
%    [XCP1,YCP1,ZCP1]=CrossSecP(theta,phic1,ratio,scl,rmin);  
   beam_temp=beam_abs(1,:);
    [XCP1,YCP1,ZCP1]=CrossSecP(theta,phic1,ratio,scl,rmin,beam_temp); 
   
   phic2=ones(size(phi))*phi(length(phi));
%    [XCP2,YCP2,ZCP2]=CrossSecP(theta,phic2,ratio,scl,rmin);    
   beam_temp=beam_abs(length(phi),:);
   [XCP2,YCP2,ZCP2]=CrossSecP(theta,phic2,ratio,scl,rmin,beam_temp);   
end
...
%     [XCP3,YCP3,ZCP3]=CrossSecP(theta,phic3,ratio,scl,rmin);   
    beam_temp=beam_abs(find(phi==phi_int(1)),:);
    [XCP3,YCP3,ZCP3]=CrossSecP(theta,phic3,ratio,scl,rmin,beam_temp);   
    
    % Plane #2
    phic4=ones(size(phi))*phi_int(2);
%     [XCP4,YCP4,ZCP4]=CrossSecP(theta,phic4,ratio,scl,rmin);  
      beam_temp=beam_abs(find(phi==phi_int(2)),:);
    [XCP4,YCP4,ZCP4]=CrossSecP(theta,phic4,ratio,scl,rmin,beam_temp);    
end

% 3D Plot
% Plot Pattern
h=mesh(X,Y,Z,'edgecolor','blue');
set(h,'linewidth',1.1);
hold on;
...
if phi(1) ~= (phi(length(phi))-2*pi)
   ...
   %Boundary Line
   bl2=plot3([0 0],[0 0],[0 min(min(Z))],'r');
   % Plot Boundary Line
...
if cor ==1
    ...
    q1=plot3([0 corr],[0 0],[0 0],'b');
    q2=plot3([0 0],[0 corr],[0 0],'b');
    q3=plot3([0 0],[0 0],[0 corr],'b');
